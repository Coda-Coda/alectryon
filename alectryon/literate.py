...
class Line:
    ...
    def isblanks(self):
        return all(p.isblanks() for p in self.parts)
...
class EmptyLine(Line):
    ...
    def __init__(self, num=-1, parts=()):
        super().__init__(num, list(parts))

def strip_deque(lines: Deque[Line]) -> Deque[Line]:
    while lines and lines[0].isblanks():
        lines.popleft()
    while lines and lines[-1].isblanks():
        lines.pop()
    return lines
...
def remove_extra_newlines(lines: Iterable[Line]):
    prev: Optional[Line] = None
    for line in strip_deque(deque(lines)):
        if isinstance(prev, EmptyLine) and isinstance(line, EmptyLine):
            # Don't collapse consecutive empty code and comment lines (to not
            # risk losing an empty literate block).
            if str(prev) == str(line):
                continue
        yield line
        prev = line

def join_lines(lines: Iterable[Line]) -> str:
    return "".join(str(l) + "\n" for l in remove_extra_newlines(lines))
